/*
 * ** mmapdemo.c -- demonstrates memory mapped files lamely.
 * */

#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
#include <unistd.h>
#include <sys/types.h>
#include <sys/mman.h>
#include <sys/stat.h>
#include <errno.h>

#define BIGFILE "/faststorage/home/runef/package_problem/onefile/131226_I114_FCC2F2MACXX_L5_SZABPI037427-15_1.fq"

int main(int argc, char *argv[])
{
	int fd;
        long offset;
	char *data;
	struct stat sbuf;

	if (argc != 2) {
		fprintf(stderr, "usage: mmapdemo offset\n");
		exit(1);
	}

	if ((fd = open(BIGFILE, O_RDONLY)) == -1) {
		perror("open");
		exit(1);
	}

	if (stat(BIGFILE, &sbuf) == -1) {
		perror("stat");
		exit(1);
	}

	offset = atol(argv[1]);
	if (offset < 0 || offset > sbuf.st_size-1) {
		fprintf(stderr, "mmapdemo: offset must be in the range 0-%ld\n", sbuf.st_size-1);
		exit(1);
	}
	
	if ((data = mmap((caddr_t)0, sbuf.st_size, PROT_READ, MAP_SHARED, fd, 0)) == (caddr_t)(-1)) {
		perror("mmap");
		exit(1);
	}

	{
		int sum = 0;
		long i;
		for (i=0; i<offset; i++) {
			sum += data[i];
		}
	}

	printf("byte at offset %ld is '%c'\n", offset, data[offset]);

	return 0;
}
